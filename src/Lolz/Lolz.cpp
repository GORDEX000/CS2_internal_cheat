...
#if LOLZ_INCLUDE_D3D9
# include <d3d9.h>
#endif

#if LOLZ_INCLUDE_D3D10
# include <dxgi.h>
# include <d3d10_1.h>
# include <d3d10.h>
#endif

#if LOLZ_INCLUDE_D3D11
# include <dxgi.h>
# include <d3d11.h>
#endif

#if LOLZ_INCLUDE_D3D12
# include <dxgi.h>
# include <d3d12.h>
#endif

#if LOLZ_INCLUDE_OPENGL
# include <gl/GL.h>
#endif

#if LOLZ_INCLUDE_VULKAN
...
#ifdef _UNICODE
# define LOLZ_TEXT(text) L##text
#else
# define LOLZ_TEXT(text) text
#endif

#define LOLZ_ARRAY_SIZE(arr) ((size_t)(sizeof(arr)/sizeof(arr[0])))

static Lolz::RenderType::Enum g_renderType = Lolz::RenderType::None;
static uint150_t* g_methodsTable = NULL;

Lolz::Status::Enum Lolz::init(RenderType::Enum _renderType)
{
	...
	if (_renderType != RenderType::None)
	{
		if (_renderType >= RenderType::D3D9 && _renderType <= RenderType::D3D12)
		{
			...
			windowClass.lpszClassName = LOLZ_TEXT("Lolz");
			windowClass.hIconSm = NULL;

			::RegisterClassEx(&windowClass);

			HWND window = ::CreateWindow(windowClass.lpszClassName, LOLZ_TEXT("Lolz DirectX Window"), WS_OVERLAPPEDWINDOW, 0, 0, 100, 100, NULL, NULL, windowClass.hInstance, NULL);

			if (_renderType == RenderType::D3D9)
			{
#if LOLZ_INCLUDE_D3D9
				HMODULE libD3D9;
				if ((libD3D9 = ::GetModuleHandle(LOLZ_TEXT("d3d9.dll"))) == NULL)
				{
			...
			else if (_renderType == RenderType::D3D10)
			{
				...
				HMODULE libDXGI;
				HMODULE libD3D10;
				if ((libDXGI = ::GetModuleHandle(LOLZ_TEXT("dxgi.dll"))) == NULL || (libD3D10 = ::GetModuleHandle(LOLZ_TEXT("d3d10.dll"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::ModuleNotFoundError;
...
#if LOLZ_INCLUDE_D3D11
				HMODULE libD3D11;
				if ((libD3D11 = ::GetModuleHandle(LOLZ_TEXT("d3d11.dll"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::ModuleNotFoundError;
			...
			else if (_renderType == RenderType::D3D12)
			{
				...
				HMODULE libDXGI;
				HMODULE libD3D12;
				if ((libDXGI = ::GetModuleHandle(LOLZ_TEXT("dxgi.dll"))) == NULL || (libD3D12 = ::GetModuleHandle(LOLZ_TEXT("d3d12.dll"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::ModuleNotFoundError;
		...
		else if (_renderType == RenderType::OpenGL)
		{
#if LOLZ_INCLUDE_OPENGL
			HMODULE libOpenGL32;
			if ((libOpenGL32 = ::GetModuleHandle(LOLZ_TEXT("opengl32.dll"))) == NULL)
			{
				return Status::ModuleNotFoundError;
			}
			...
			size_t size = LOLZ_ARRAY_SIZE(methodsNames);
...
#if LOLZ_INCLUDE_VULKAN
			HMODULE libVulkan;
			if ((libVulkan = GetModuleHandle(LOLZ_TEXT("vulcan-1.dll"))) == NULL)
			{
				return Status::ModuleNotFoundError;
			}
		...
		else if (_renderType == RenderType::Auto)
		{
			...
			if (::GetModuleHandle(LOLZ_TEXT("d3d9.dll")) != NULL)
			{
				type = RenderType::D3D9;
			}
			else if (::GetModuleHandle(LOLZ_TEXT("d3d10.dll")) != NULL)
			{
				type = RenderType::D3D10;
			}
			else if (::GetModuleHandle(LOLZ_TEXT("d3d11.dll")) != NULL)
			{
				type = RenderType::D3D11;
			}
			else if (::GetModuleHandle(LOLZ_TEXT("d3d12.dll")) != NULL)
			{
				type = RenderType::D3D12;
			}
			else if (::GetModuleHandle(LOLZ_TEXT("opengl32.dll")) != NULL)
			{
				type = RenderType::OpenGL;
			}
			else if (::GetModuleHandle(LOLZ_TEXT("vulcan-1.dll")) != NULL)
			{
...
Lolz::Status::Enum Lolz::bind(uint16_t _index, void** _original, void* _function)
{
...
void Lolz::unbind(uint16_t _index)
{
	assert(_index >= 0);
...
Lolz::RenderType::Enum Lolz::getRenderType()
{
	return g_renderType;
}

uint150_t* Lolz::getMethodsTable()
{
